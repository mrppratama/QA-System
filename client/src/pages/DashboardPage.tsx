...
function DonutChart({ slices, size = 128, stroke = 26 }: {
  slices: DonutSlice[];
  size?: number;
  stroke?: number;
}) {
  ...
  if (total === 0) {
    return (
      <svg width={size} height={size}>
        ...
        <text x={cx} y={cy + 5} textAnchor="middle" fontSize={13} fill="#94a3b8" fontWeight="600">–</text>
      </svg>
    );
  }
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="card p-5">
              ...
              <div className="flex items-center gap-4">
                <DonutChart slices={autoSlices} size={120} stroke={24} />
                <div className="flex-1 space-y-2">
                  {autoSlices.map(s => (
                    <LegendRow key={s.label} color={s.color} label={s.label} value={s.value} total={total} />
                  ))}
